...
export default function ChatRoom() {
  ...
  const sendMessage = async (e) => {
    ...
    try {
      ...
      setError("Failed to send message. Check Firestore rules.");
    }
  };
    ...
    <div className="bg-zinc-900 border border-gray-700 p-6 rounded-xl shadow-lg max-w-xl mx-auto mt-5">
      ...
      {error && (
        <div className="bg-red-500/20 border border-red-500 text-red-200 p-3 rounded-lg mb-4 text-sm">
          ...
          {error.includes("configuration-not-found") && (
            <p className="mt-1">
              Please enable Google Auth in your Firebase Console (Authentication &gt; Sign-in method).
            </p>
          )}
        </div>
      )}
            ...
            <div
              className={`p-3 rounded-lg max-w-[75%] ${
                msg.uid === user?.uid
                  ...
                  : "bg-zinc-700 text-white border border-gray-600"
              }`}
            >
              <div className="text-[10px] opacity-70 mb-1">{msg.displayName}</div>
      ...
      {user ? (
        <form onSubmit={sendMessage} className="flex gap-2 flex-wrap sm:flex-nowrap w-full">
          ...
          <button
            type="submit"
            className="bg-blue-600 px-4 py-2 rounded-lg text-white hover:bg-blue-700 w-full sm:w-auto transition-colors"
          >
            Send
          </button>
        </form>
        ...
        <div className="flex flex-col items-center justify-center gap-4">
          <button
            onClick={handleLogin}
            disabled={loading}
            className={`flex items-center gap-3 bg-white text-gray-800 px-6 py-2.5 rounded-full shadow-lg hover:bg-gray-100 transition-all ${loading ? 'opacity-50 cursor-not-allowed' : ''}`}
          >
            {loading ? (
          ...
          <p className="text-sm text-gray-400">Login to participate in the chat</p>
        </div>
      )}
    </div>
  );
